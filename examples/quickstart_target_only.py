...
# ── Synthetic hourly load (2 years, 2 zones) ─────────────────────────────────
...
val_df = df[df["timestamp"] >= timestamps[split - 96]].copy()   # small overlap for context
...
# ── Load model ────────────────────────────────────────────────────────────────
model = load_toto2("Datadog/Toto-2.0-22M")
...
print(f"\nFirst loss: {first_loss:.4f}  →  Final loss: {final_loss:.4f}")
...
print(f"\nq_preds shape: {q_preds.shape}  (Q, N, n_target, H)")
print(f"P50 mean: {q_preds[4].mean():.2f}  (expect ~1500 for synthetic load)")
